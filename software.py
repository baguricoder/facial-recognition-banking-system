...
class BankUi:
    def __init__(self,root):
        self.root = root
        self.header = Label(self.root,text="SVCET BANK OF CHITTOR",bg="#0019fc",fg="white",font=("arial",20,"bold"))
    ...
    confirm_password = self.confirm_pentry.get()  # Assuming confirm password entry is called confirm_pentry
    ...
    if not name and not password:
        ...
        self.enroll_user()
    elif password != confirm_password:  # Check if password and confirm password match
        messagebox._show("Password Mismatch", "The passwords do not match! Please try again.")
        self.enroll_user()
    else:
        self.write_to_csv()  # Save the account info (assuming the method exists)
        self.video_capture_page()  # Transition to the next screen
